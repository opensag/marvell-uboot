...
part_entry efi_part[3];

int part_efi(struct blk_desc *dev_desc)
{
	...
	for (i = 0; i < le32_to_cpu(gpt_head->num_partition_entries); i++) {
		...
		efi_part[i].start_lpa = gpt_pte[i].starting_lba;
		efi_part[i].end_lpa = gpt_pte[i].ending_lba;
		strcpy(efi_part[i].part_name, print_efiname(&gpt_pte[i]));
		...
		disk_partition_t *partitions, int parts_count)
{
	...
	printf("gpt_fill_header ret=%x\n", ret);
	if (ret)
		goto err;

	/* Generate partition entries */
	ret = gpt_fill_pte(gpt_h, gpt_e, partitions, parts_count);
	printf("gpt_fill_pte ret=%x\n", ret);
	if (ret)
		goto err;

	/* Write GPT partition table */
	ret = write_gpt_table(dev_desc, gpt_h, gpt_e);
	printf("write_gpt_table ret=%x\n", ret);
