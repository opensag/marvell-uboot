...
static int mmc_burn_image(size_t image_size)
{
...
#endif
	printf("mmc_dev_num=%x %x %x\n", mmc_dev_num, CONFIG_SYS_MMC_ENV_DEV, CONFIG_SYS_MMC_ENV_PART);
	...
	blk_written = blk_dwrite(blk_desc, start_lba, blk_count,
				 (void *)get_load_addr());
	printf("v=%x start_lba=%llx blk_count=%llx add=%lx , blk_written %lx\n", 
						mmc->version, start_lba, blk_count, get_load_addr(), blk_written);
#else
...
int do_bubt_cmd_mmc(cmd_tbl_t *cmdtp, int flag, int argc, char * const argv[])
{
	size_t add,len;
	char cmd[128];
	if (argc < 3) 
	...
	add = simple_strtoul(argv[1], NULL, 16);
	sprintf(cmd, "setenv loadaddr 0x%lx", add);
	run_command(cmd, 0);
	len = simple_strtoul(argv[2], NULL, 16);

	return mmc_burn_image(len);
}


U_BOOT_CMD(
	but_mmc, 4, 0, do_bubt_cmd_mmc,
	"Burn a u-boot image to mmc from addrass",
	"address\n"
);
