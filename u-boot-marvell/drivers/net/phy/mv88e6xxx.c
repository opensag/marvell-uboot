		...
		unsigned short val)
{
	int ret;
	const char *name = miiphy_get_current_dev();
	//printf("mv88e6xxx_write_register port=%x reg=%x val=%x \n", port, reg, val);
			...
			int reg, unsigned short val)
{
	...
	//printf("mv88e6xxx_write_phy_register port=%x page=%x, reg=%x, val=%x \n",
	//										port, page, reg, val );
...
int mv88e6xxx_initialize(const void *blob)
{
	...
	if (soho_dev.cpu_port != -1) {
		...
		ret = mv88e6xxx_write_register(&soho_dev,
					       REG_PORT(soho_dev.cpu_port),
					       PORT_PCS_CTRL, reg);
		//printf("mv88e6xxx_write_register ret=%x\n", ret);
		if (ret)
			return ret;
	}
	//printf("soho_dev.port_mask=%x %x\n", soho_dev.port_mask, soho_dev.cpu_port);
	/* Force port setup */
	for (port = 0; port < sizeof(soho_dev.port_mask) * 8; port++) {
		...
		//printf("port=%x %x =%lx\n",
		//		port, soho_dev.port_mask  , (soho_dev.port_mask & ~BIT(port)) );
